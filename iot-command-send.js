...
module.exports = function(RED) {
    ...
    function GoogleCloudIoTCommandSendNode(config) {
        ...
        /*
        if (!config.projectId) {
            node.error('No project id supplied!');
            return;
        }

        if (!config.region) {
            node.error('No region supplied!');
            return;
        }

        if (!config.registryId) {
            node.error('No registry id supplied!');
            return;
        }

        if (!config.deviceId) {
            node.error('No device id supplied!');
            return;
        }
        */
        ...
        async function OnInput(msg) {
            ...
            try {
                ...
                try {
                    const responses = await iotDeviceManagerClient.sendCommandToDevice(request);
                } catch (err) {
                    node.error(`Could not send command: ${err}`);
                }
    ...
    } // GoogleCloudIoTCommandSendNode

    RED.nodes.registerType("google-cloud-iot command-send", GoogleCloudIoTCommandSendNode);
};
